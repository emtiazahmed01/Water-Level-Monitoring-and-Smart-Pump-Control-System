...
#define BLYNK_TEMPLATE_ID "TMPL6K9HcA1Me"
#define BLYNK_TEMPLATE_NAME "Smart Tank  Monitor""
...
// Changed trig pin from 12 to 26 to avoid ESP32 boot strapping locks
#define trig 12
#define echo 13
#define relay 14
...
// Calculate thresholds based on actual WATER DEPTH (0% to 100%)
int Level1 = (MaxLevel * 25) / 100; // 25% Full 
int Level2 = (MaxLevel * 50) / 100; // 50% Full 
int Level3 = (MaxLevel * 75) / 100; // 75% Full 
int Level4 = (MaxLevel * 90) / 100; // 90% Full (Leaves a small safety buffer at the top!)
...
// Insert your actual Auth Token from Blynk here
char auth[] = "wQg17qPFF5QUaMoAvxONIfKHhNLGY-oo";

// Wokwi Virtual Wi-Fi Network Credentials
...
void setup() {
  ...
  // Connects Wokwi to real Blynk servers via Wokwi-GUEST network
...
void ultrasonic() {
  ...
  // blynkDistance represents the actual depth of the water
  ...
  // Evaluate from top to bottom based on water depth
  if (blynkDistance >= Level4) {
    ...
    // --- NEW AUTOMATIC PUMP SHUT-OFF LOGIC ---
    digitalWrite(relay, HIGH);             // Physically turn off the pump
    setLine(lineBottom, "Motor is OFF");   // Update the physical LCD screen
    Blynk.virtualWrite(V1, 0);             // Sync the Blynk app toggle switch to OFF
    // -----------------------------------------
